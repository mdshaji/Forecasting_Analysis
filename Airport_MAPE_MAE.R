...
library(tseries)
library(readr)
Airline <- read_csv("D:/360 Digitmg - Assignments/Module-Forecasting/Codes/Airlines Data.csv")
View(Airline)
...
ma_model1 <- sma(train)
ma_pred <- data.frame(predict(ma_model1, h = 16))
ma_pred
...
hw_a
hwa_pred <- data.frame(predict(hw_a, n.ahead = 16))
...
hw_abg
hwabg_pred <- data.frame(predict(hw_abg, n.ahead = 16))
...
hw_na
hwna_pred <- data.frame(predict(hw_na, n.ahead = 16))
hwna_pred
plot(forecast(hw_na,h=16))
hwna_mape <- MAPE(hwna_pred$fit,test)*100


hw_nab <- HoltWinters(train, gamma = F)
hw_nab
hwnab_pred <- data.frame(predict(hw_nab, n.ahead = 16))
hwnab_pred
plot(forecast(hw_nab, h = 16))
hwnab_mape <- MAPE(hwnab_pred$fit, test)*100

hw_nabg <- HoltWinters(train)
hw_nabg
hwnabg_pred <- data.frame(predict(hw_nabg, n.ahead = 16))
hwnabg_pred
plot(forecast(hw_nabg, h = 16))
...
forecast_new <- data.frame(predict(new_model, n.ahead = 16))
forecast_new
########################################################################
#
#
...
ses_a
sesa_pred<-data.frame(predict(ses_a,h=16))
plot(forecast(ses_a,n.ahead=16))
...
# Fore casted values for the next 4 quarters
